...
export class TWAPAggregation extends BaseAggregation {
  ...
  fetchPrice(): f64 {
    ...
    const sourceData = this.redisClient.get(this.source.id + '_twap')
  ...
  fetchTwapData(): TwapData {
    ...
    const twapResponse = this.redisClient.get(this.source.id + '_twap')
  ...
  saveTwapData(twapData: TwapData): boolean {
    this.redisClient.set(this.source.id + '_twap', twapData.toString())
    return true
  }
